...
func StartAdmin() {
	...
	r := mux.NewRouter()
	r.Use(recoverHttp, authMiddleware, limitBodyMiddleware, handlers.CompressHandler)
	// 所有路由添加安全头
	...
	// 公开端点(无需认证)
	portal.HandleFunc("/login", UserPortalLogin).Methods(http.MethodPost).Name("login")
	portal.HandleFunc("/password/request_reset", UserPortalRequestPasswordReset).Methods(http.MethodPost).Name("login")
	portal.HandleFunc("/password/reset", UserPortalResetPassword).Methods(http.MethodPost).Name("login")
	portal.HandleFunc("/password/policy", UserPortalGetPasswordPolicy).Methods(http.MethodGet).Name("login")
	// 需要用户认证的端点
	portalAuth := portal.PathPrefix("").Subrouter()
	portalAuth.Use(portalAuthMiddleware)
	portalAuth.HandleFunc("/profile", UserPortalProfile).Methods(http.MethodGet)
	portalAuth.HandleFunc("/password/change", UserPortalChangePassword).Methods(http.MethodPost)
	portalAuth.HandleFunc("/login_history", UserPortalLoginHistory).Methods(http.MethodGet)
	portalAuth.HandleFunc("/active_sessions", UserPortalActiveSessions).Methods(http.MethodGet)
	portalAuth.HandleFunc("/session/disconnect", UserPortalDisconnectSession).Methods(http.MethodPost)
	portalAuth.HandleFunc("/otp/status", UserPortalGetOtpStatus).Methods(http.MethodGet)
	portalAuth.HandleFunc("/otp/bind", UserPortalBindOtp).Methods(http.MethodPost)
	portalAuth.HandleFunc("/otp/confirm", UserPortalConfirmOtp).Methods(http.MethodPost)
	portalAuth.HandleFunc("/otp/reset", UserPortalResetOtp).Methods(http.MethodPost)
	portalAuth.HandleFunc("/otp/recovery_codes", UserPortalRegenerateRecoveryCodes).Methods(http.MethodPost)
	portalAuth.HandleFunc("/bandwidth", UserPortalBandwidthStats).Methods(http.MethodGet)
	portalAuth.HandleFunc("/routes", UserPortalGetRoutes).Methods(http.MethodGet)
